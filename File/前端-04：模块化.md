...
![](https://cdn.nlark.com/yuque/0/2022/png/21983605/1657779119673-8aec96c5-f582-47a2-9bb3-8f9d6e33ef14.png#alt=%E5%89%8D%E7%AB%AF.png)
...
- 
先在a.js中创建一个值为20的变量x，并创建能接受两个参数并返回包含x和这两个参数的从大到小排序数组的函数。在b.js中创建一个值为2的变量作为权重w，再创建如下函数：
```javascript
function arith(arr){
...
两个js文件分别用默认导出和按需导出的方式暴露必要的变量或者函数。
...
在index.html中输入如下代码生成7个li标签：
```html
ul>li*7{我是第$个小li}
```


在index.js中npm下载并利用jQuery插件让li标签的奇行和偶行背景颜色分别为粉色和天蓝色，增加index.html对其的引用，再预览html页面，你发现了什么？
